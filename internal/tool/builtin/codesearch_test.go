...
func TestCodeSearchFormatsPathAndLineRange(t *testing.T) {
	...
	got, err := NewCodeSearch(ix, nil).Execute(context.Background(), json.RawMessage(`{"request":"validacion de sesion"}`))
	if err != nil {
...
func TestCodeSearchRequiresRequest(t *testing.T) {
	...
	if _, err := NewCodeSearch(ix, nil).Execute(context.Background(), json.RawMessage(`{"request":"  "}`)); err == nil {
...
func TestCodeSearchReportsEmptyIndexWithoutFailing(t *testing.T) {
	...
	got, err := NewCodeSearch(ix, nil).Execute(context.Background(), json.RawMessage(`{"request":"lo que sea"}`))
	if err != nil {
...
func TestCodeSearchCapsLimit(t *testing.T) {
	...
	if _, err := NewCodeSearch(ix, nil).Execute(context.Background(), json.RawMessage(`{"request":"x","limit":9999}`)); err != nil {
		t.Fatal(err)
	}
...
func TestCodeSearchPassesPathPrefix(t *testing.T) {
	...
	ix := &fakeIndex{chunks: 10}
	if _, err := NewCodeSearch(ix, nil).Execute(context.Background(), json.RawMessage(`{"request":"x","path":"internal/agent/"}`)); err != nil {
		t.Fatal(err)
	}
...
func TestCodeSearchNoMatchesSuggestsGrep(t *testing.T) {
	...
	ix := &fakeIndex{chunks: 10}
	got, err := NewCodeSearch(ix, nil).Execute(context.Background(), json.RawMessage(`{"request":"algo que no existe"}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCodeSearchSurfacesIndexError(t *testing.T) {
	...
	if _, err := NewCodeSearch(ix, nil).Execute(context.Background(), json.RawMessage(`{"request":"x"}`)); err == nil {
...
func TestCodeSearchIsReadOnly(t *testing.T) {
	...
	if !NewCodeSearch(&fakeIndex{chunks: 1}, nil).ReadOnly() {
