...
func TestNewProviderRejectsExplicitOfficialDeepSeekVisionModel(t *testing.T) {
	...
	// El endpoint oficial no recibe la imagen aunque los metadatos digan que
	// tiene vision. Este fork no la borra en silencio: conserva el texto y le
	// suma donde quedo guardada, para que se pueda pedir con read_image.
	got, ok := message["content"].(string)
	if !ok || !strings.HasPrefix(got, "describe") {
		t.Fatalf("content = %#v, want the original text despite stale explicit vision metadata", message["content"])
	}
	if !strings.Contains(got, "read_image") {
		t.Fatalf("la imagen desaparecio sin dejar como recuperarla: %q", got)
	}
